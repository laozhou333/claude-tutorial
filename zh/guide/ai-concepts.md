...
# 看起来只有 3 行代码，但实际上有大约 15-20 个 Token
# 因为空格、缩进、括号、引号都会被拆分成独立的 Token
```

::: info 小贴士
Token 的切分因模型而异，无法手动精确计算。你可以用 [Anthropic Tokenizer](https://docs.anthropic.com/en/docs/build-with-claude/token-counting) 查看精确计数。关键是理解：**Token ≠ 字符 ≠ 单词**，它是介于两者之间的单位。
:::
